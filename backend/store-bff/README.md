...
它是本仓库的**端 BFF 之一**（另一个是 admin）：只持**店主账号 `store_user`**，负责注册/登录/签发与**跨服务编排**——店铺数据在 store 域、商品分类品牌在中台（goods-center），本模块自己不落这些表。

## 一、架构位置

**端 BFF（第 ① 层）**：公网唯一入口是网关；本层的 `/auth/**`、`/shops/**`、`/goods/**` 就是**前端唯一可见的接口面**。
...
> 📋 对外接口清单（17 条）见 [`docs/contracts/store-bff.md`](../../docs/contracts/store-bff.md)。
...
- 签发 `type=store` 的 JWT 并写 Redis 店主登录上下文，**键 = `panoramic:login:store:{userId}`**（各端身份空间隔离，2026-09-10 起）
...
| 门禁 | 口径 |
|---|---|
| **店铺已审核通过（R9）** | 商品接口（**含读接口**）全部先进 `StoreGoodsBffService#assertShopApprovedAndGetStoreId()`——经 `StoreClient.mineShop(storeId)` 校验店铺 `status == 2`，否则 `code=403`「店铺未通过审核，暂不可管理商品」。**域内不做该判断** |
| **中台版本同步（R10）** | 详情里若 `goodsSpuId != null`，调 `GoodsCenterClient.spuDetail()` 取中台当前 `version`，与落库 `center_version` 比对——不等则 `centerOutdated=true` 并附 `centerSpu` 供前端「同步」覆盖表单（覆盖与否**由店主决定，不阻断保存**）；中台已删/不可达则 `centerMissing=true`（**不报错**） |
| **分类全路径读时解析（D4/D9）** | 域不持分类表，只存「分类 id 引用 + 名称快照」，故 `categoryPath`（如「服饰 / 男装 / T恤」）在列表/详情里**读时**调 `GoodsCenterClient.categoryPaths(ids)` 补全——列表按**页内去重后的分类 id 发一次**请求（非 N+1）。⚠ **降级隔离**：包在独立 `try/catch`，中台不可用/熔断只记 `warn` 并让 `categoryPath` 留空，列表与详情照常返回，前端回退显示快照 `categoryName`——**展示增强绝不允许拖垮主流程** |
