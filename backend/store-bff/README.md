...
它是本仓库**端 BFF 之一**（另两个是 admin / mall-bff）：只持**店主账号 `store_user`**，负责注册/登录/签发与**跨服务编排**——店铺数据在 store 域、商品分类品牌在中台（goods-center）、订单在交易域（trade-center），本模块自己不落这些表。
...
- `StoreShopBffService` / `StoreGoodsBffService` / `StoreOrderBffService`（`com.panoramic.storebff.bff`）**只做编排**，不持域实体：分别持 `StoreClient`（商品那个另持 `GoodsCenterClient`，订单那个持 `TradeCenterClient`）。
...
- `@EnableFeignClients(basePackages = {"com.panoramic.contract.store", "com.panoramic.contract.goods", "com.panoramic.contract.trade"})` 扫描内部 Feign 客户端（store 域 + goods-center + trade-center）
