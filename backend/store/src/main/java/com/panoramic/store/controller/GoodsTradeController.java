 ...
 * <p>出参直接返业务类型（<b>不包 {@code RespData}</b>），错误经 {@code StoreDomainExceptionHandler}
 * 以真实 HTTP 状态码传播：业务 4xx 由调用方熔断忽略，5xx 计入失败率。唯一的「非错误失败」是
 * {@link #deductStock} 的 {@code false}——库存不足是正常业务结果（R18），不是 5xx。</p>
...
public class GoodsTradeController {
    ...
    public List<StoreGoodsSkuSnapshotVO> tradeSkuSnapshotBatch(
            @Validated @RequestBody StoreGoodsSkuBatchQueryDTO dto) {
        return storeGoodsSpuService.platformSkuSnapshotBySkuIds(dto.getSkuIds());
    }

    /**
     * 扣减库存：成功 {@code true}；库存不足 {@code false}（HTTP 200，不是错误）。
     * <p>不判锁定、不判上下架、不校验店铺归属（可见性由交易侧判，R19）。</p>
     */
    @PostMapping("/stock/deduct")
    public boolean deductStock(@Validated @RequestBody StoreStockDeductDTO dto) {
        return storeGoodsSkuStockService.deduct(dto.getSkuId(), dto.getQuantity(), dto.getOrderNo());
    }
    ...
    public void revertStockByOrder(@PathVariable("orderNo") String orderNo) {
