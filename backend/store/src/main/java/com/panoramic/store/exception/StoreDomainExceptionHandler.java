package com.panoramic.store.exception;
...
/**
 * 店铺域（内部接口）异常处理器。
 * <p>store 域已下沉为纯域、无页面消费者，统一把异常还原为<b>真实 HTTP 状态码</b>
...
public class StoreDomainExceptionHandler {
    ...
    public ResponseEntity<RespData<Void>> handleServiceException(ServiceException e) {
        log.warn("店铺域业务异常: code={}, msg={}", e.getCode(), e.getMessage());
