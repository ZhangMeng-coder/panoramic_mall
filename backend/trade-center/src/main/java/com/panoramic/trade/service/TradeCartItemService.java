 ...
 * 数据权限锚点 {@code customerId = mall_user.id}：八个方法**全按传入锚点过滤**，锚点即数据权限本身；
...
public interface TradeCartItemService extends IService<TradeCartItem> {
     ...
     */
    void updateQuantity(Long customerId, Long id, TradeCartItemUpdateDTO dto);
     ...
     * @throws com.panoramic.common.exception.ServiceException 行不存在或不属于该顾客（HTTP 404）
     */
    void setItemSelected(Long customerId, Long id, TradeCartSelectDTO dto);
     ...
     * @param dto        选中状态
     */
    void setAllSelected(Long customerId, TradeCartSelectDTO dto);

    /**
     * 批量删除购物车行（<b>物理删除</b>；<b>幂等</b>：删 0 行不报错）。
     *
     * @param customerId 顾客账号 id（数据权限锚点）
     * @param dto        待删除的行 id 列表（空列表直接返回）
     */
    void removeItems(Long customerId, TradeCartItemIdsDTO dto);
