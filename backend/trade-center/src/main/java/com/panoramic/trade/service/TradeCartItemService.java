 ...
 * ⚠ **六个写方法**（五条走 DTO + {@code clearCart}）的作用域**缺失即 400**（域应用层断言，
 * 见 {@code com.panoramic.trade.support.ScopeGuard}）：DTO 上的 {@code @NotNull} 只覆盖 MVC 边界，
 * 绕过 MVC 的调用只有那道断言能挡——{@code null} 到了 SQL 层就变成「不限定」，会**静默**改到别人的行。
 * （{@code listItems} / {@code countItems} 是读，作用域缺失即「不限定」，不在此列。）
