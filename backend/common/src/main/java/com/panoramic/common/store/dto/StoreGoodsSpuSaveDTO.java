package com.panoramic.common.store.dto;

import com.panoramic.common.goods.dto.SpecConfigItem;
import jakarta.validation.Valid;
import jakarta.validation.constraints.NotBlank;
import jakarta.validation.constraints.NotNull;
import jakarta.validation.constraints.Size;
import lombok.Data;

import java.util.List;

/**
 * 店铺在售商品（SPU）新增请求参数（store 域内部接口与 store-bff 同源共享）。
 * <p>字段与中台标准商品同构；分类必填、品牌非必填。分类/品牌名称以「快照」形式随 id 一并提交落库
 * （下拉数据来自中台，保存时不再回查中台——「分类全路径」由端 BFF 在读取时另行调 goods-center 解析，
 * 不落在写入链路上）。商品上下架状态由 SKU 联动推导，不接受前端传入。</p>
 */
@Data
public class StoreGoodsSpuSaveDTO {

    /**
     * 商品名称
     */
    @NotBlank(message = "商品名称不能为空")
    @Size(max = 128, message = "商品名称不能超过128个字符")
    private String name;

    /**
     * 所属分类ID（引用中台 goods_category）
     */
    @NotNull(message = "商品分类不能为空")
    private Long categoryId;

    /**
     * 分类名称快照
     */
    @Size(max = 64, message = "分类名称不能超过64个字符")
    private String categoryName;

    /**
     * 品牌ID（引用中台 goods_brand，非必填）
     */
    private Long brandId;

    /**
     * 品牌名称快照
     */
    @Size(max = 64, message = "品牌名称不能超过64个字符")
    private String brandName;

    /**
     * 主图 URL
     */
    @Size(max = 255, message = "主图地址长度不能超过255个字符")
    private String mainImage;

    /**
     * 轮播图 URL 列表
     */
    private List<String> imageList;

    /**
     * 商品详情（富文本 HTML）
     */
    private String description;

    /**
     * 规格属性配置（结构校验由服务层完成）
     */
    private List<SpecConfigItem> specConfig;

    /**
     * 中台关联 SPU id（可空；填 SKU_CODE 预填时由中台返回）
     */
    private Long goodsSpuId;

    /**
     * 中台 SPU 版本戳（可空；点过「同步」后随保存提交，落 center_version）
     */
    private Long centerVersion;

    /**
     * SKU 列表（可空 = 暂无 SKU；SKU 一律以下架态落库）
     */
    @Valid
    private List<StoreGoodsSkuDTO> skus;
}
