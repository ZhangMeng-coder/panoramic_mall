 ...
 *       用户类型（X-User-Type）<b>原样透传</b>给下游（优先取当前 Web 请求上 gateway 透传的身份头，经
 *       RequestContextHolder 跨熔断线程读取；兜底取请求线程 UserContext），供 store 域做平台/店主分流；
 *       ⚠ 不做 goods 版「缺省回退 admin」的写死兜底——店主侧（store-bff）调用若被盖成 admin，
 *       会使 store 域 owner 分支（X-User-Type=store）失效，故缺省就缺省，交给下游校验；</li>
