...
- 顾客账号 `mall_user`（手机号 / 密码 / 登录态）归 **mall-bff**（见 [`../mall-bff/README.md`](../mall-bff/README.md)）；本域**不持顾客账号**、不校验 token、不与账号表联查
...
- **账号与资料拆表**：手机号/密码留 `mall_user`（mall-bff），昵称/头像/性别/生日落 `customer_profile`（本域）；资料行随注册首建（主键 = 账号 id）
- **省市区 = 自由文本单列**（`region`，与 `store_shop.region` 同口径）——**不建地区表**、不做省市区三级联动数据
- **默认地址唯一**：同一顾客至多一条 `is_default=1`，由应用层在**同一事务内**先清位再置位（`setDefaultAddress` 另先对该顾客名下行加锁，串行化并发设默认）；**删掉默认地址后不自动递补**（其余地址保持非默认）。
  ⚠ **已知窗口（未关）**：地址簿为空时（新注册，或把地址全删光）两笔并发新增会**双双成为默认**——空地址簿无行可锁、间隙锁彼此兼容，应用层关不上；彻底解法是 DB 层部分唯一索引（MySQL 8 函数索引），属表结构选型，**未在本批次采纳**
- **设默认只有两条路径**：首条地址自动设为默认、`setDefaultAddress`——故 `CustomerAddressSaveDTO` **不含 `isDefault`**（新增/编辑不能直接传）
