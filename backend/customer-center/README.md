...
| C3 | **默认地址唯一**：同一顾客至多一条 `is_default=1`，由应用层在**同一事务内**先清位再置位（`setDefaultAddress` 另先对该顾客名下行加锁，串行化并发设默认）；**删掉默认地址后不自动递补**（其余地址保持非默认）。⚠ **已知窗口（未关）**：地址簿为空时（新注册，或按 D7 把地址全删光）两笔并发新增会**双双成为默认**——空地址簿无行可锁、间隙锁彼此兼容，应用层关不上；彻底解法是 DB 层部分唯一索引（MySQL 8 函数索引），属表结构与 spec §6.4 选定的「应用层事务」设计，**未在本批次采纳** |
