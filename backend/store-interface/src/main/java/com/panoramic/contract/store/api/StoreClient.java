...
public interface StoreClient {
     ...
     * <p>⚠ 查不到一律<b>返空</b>（{@code code=200} + {@code data=null} → 调用方解包得 {@code null}），域内不抛；
     * 「未开店」是正常态，不是故障。调用方各自重判：store-bff 判「未开店」、admin 转本层
     * 「店铺不存在」、mall-bff 判「店铺不可见」（见 store.md 第三节）。
     */
    @GetMapping("/shops/{id}")
    RespData<ShopVO> getShop(@PathVariable("id") Long id);

    /**
     * 保存草稿（{@code dto.storeId} 必填：无店则建 id=storeId 的店；待审核/已通过状态机守卫）
     */
    @PostMapping("/shops/save")
    RespData<Void> saveShop(@RequestBody ShopSaveDTO dto);

    /**
     * 提交审核（{@code dto.storeId} 必填：完整资质校验 → 待审核）
     */
    @PostMapping("/shops/submit")
    RespData<Void> submitShop(@RequestBody ShopSaveDTO dto);

    /**
     * 店铺分页（状态/关键字筛选，全量——本能力存在合法全量视角，故无作用域字段）
     */
    @GetMapping("/shops/page")
    RespData<PageResult<ShopVO>> pageShops(@SpringQueryMap ShopPageQueryDTO dto);

    /**
     * 审核通过/驳回（仅对待审核条件更新）
     */
    @PostMapping("/shops/{id}/audit")
    RespData<Void> auditShop(@PathVariable("id") Long id, @RequestBody ShopAuditDTO dto);
    ...
    RespData<PageResult<StoreGoodsSpuPageItemVO>> pageStoreGoods(@SpringQueryMap StoreGoodsSpuPageQueryDTO dto);

    /**
     * 店铺商品详情（含 SKU 列表与锁定信息）。
     * <p>作用域 {@code query.storeId} <b>可空</b>（该能力存在合法全量视角）：传了就按它筛
     * （店主侧传自己的店，不属本店与不存在同样回 {@code code=400}、不泄露存在性），没传就是不限定（管理端 / C 端跨店详情）。
    ...
    RespData<StoreGoodsSpuPlatformDetailVO> storeGoodsDetail(@PathVariable("id") Long id,
    ...
    RespData<Long> saveStoreGoods(@RequestBody StoreGoodsSpuSaveDTO dto);

    /**
     * 修改商品（{@code dto.storeId} 必填；存在上架 SKU 时规格配置只读；centerVersion 非空则刷新关联版本戳）
     */
    @PutMapping("/goods/spu/{id}")
    RespData<Void> updateStoreGoods(@PathVariable("id") Long id, @RequestBody StoreGoodsSpuUpdateDTO dto);
    ...
    RespData<Void> deleteStoreGoods(@PathVariable("id") Long id, @RequestParam("storeId") Long storeId);

    /**
     * SKU 整单替换（{@code dto.storeId} 必填；未上架可增/改/删；已上架必须原样保留且不得缺失）
     */
    @PutMapping("/goods/spu/{id}/skus")
    RespData<Void> replaceStoreGoodsSkus(@PathVariable("id") Long id, @RequestBody StoreGoodsSkuReplaceDTO dto);

    /**
     * SKU 上下架（{@code dto.storeId} 必填；驱动所属 SPU 状态联动）
     */
    @PutMapping("/goods/spu/{spuId}/skus/{skuId}/shelf")
    RespData<Void> updateStoreGoodsSkuShelf(@PathVariable("spuId") Long spuId, @PathVariable("skuId") Long skuId,
    ...
    RespData<PageResult<StoreGoodsStockPageItemVO>> pageSkuStock(@SpringQueryMap StoreGoodsStockPageQueryDTO dto);
    ...
    RespData<Void> updateSkuStock(@PathVariable("skuId") Long skuId, @RequestBody StoreGoodsStockUpdateDTO dto);
    ...
    RespData<PageResult<StoreGoodsSpuCrossShopPageItemVO>> pageStoreGoodsCrossShop(
    ...
    RespData<StoreGoodsSpuFacetVO> crossShopFacets(@RequestBody StoreGoodsSpuFacetQueryDTO dto);
     ...
     * {@link #storeGoodsDetail} 取不到即 {@code code=400} 的语义不同：批量场景逐行报错会让整个列表
    ...
    RespData<List<StoreGoodsSpuPlatformDetailVO>> batchSpuDetail(@RequestBody StoreGoodsSpuBatchQueryDTO dto);

    /**
     * 锁定商品（原因必填）：写锁定字段 + 名下 SKU 级联下架 → SPU 推导为下架
     */
    @PostMapping("/goods/spu/{id}/lock")
    RespData<Void> lockStoreGoods(@PathVariable("id") Long id, @RequestBody StoreGoodsLockDTO dto);

    /**
     * 解锁商品：清空锁定字段；<b>不恢复上架</b>（SKU 保持下架，需店主手动上架）
     */
    @PostMapping("/goods/spu/{id}/unlock")
    RespData<Void> unlockStoreGoods(@PathVariable("id") Long id);
     ...
     * <b>{@code code=400}「该商品已评价」</b>（不静默改写、不吞掉）。</p>
     * <p>写入成功后<b>同一事务内</b>重算并回写该商品与所属店铺的评分冗余列。</p>
     */
    @PostMapping("/goods/evaluation")
    RespData<Void> submitEvaluation(@RequestBody StoreGoodsEvaluationSubmitDTO dto);
    ...
    RespData<PageResult<StoreGoodsEvaluationPageItemVO>> pageEvaluations(@RequestBody StoreGoodsEvaluationPageQueryDTO dto);
    ...
    RespData<List<Long>> listEvaluatedSpuIds(@PathVariable("orderNo") String orderNo,
     ...
     * 以「回复列为空」为条件更新，<b>影响行数是唯一判据</b>：已回复过 → {@code code=400}「该评价已回复」。</p>
     */
    @PostMapping("/goods/evaluation/{id}/reply")
    RespData<Void> replyEvaluation(@PathVariable("id") Long id, @RequestBody StoreGoodsEvaluationReplyDTO dto);
    ...
    // ⚠ 库存不足**不是错误**（R18）：deductStock 返 code=200 + data=false，由交易域翻成业务错误；
    //   按单回补是补偿路径，**宁可不做也不能炸**（R19）——入参空 / 该单没扣过一律 no-op，不抛异常。
    // ⚠ 熔断口径与其它域调用一致：只有 HTTP 5xx / 连接失败计入失败率，业务失败已不抛异常（第 13 条）。
     ...
     * 与 {@link #storeGoodsDetail} 取不到即 {@code code=400} 刻意不同：下单流水线要自己判「拿不到 = 商品不存在、
    ...
    RespData<List<StoreGoodsSkuSnapshotVO>> tradeSkuSnapshotBatch(@RequestBody StoreGoodsSkuBatchQueryDTO dto);

    /**
     * 扣减库存（下单出库）：库存表上一条原子条件更新，<b>影响行数是唯一判据</b>。
     * <p>调用方是 <b>trade-center 的库存适配器</b>。扣减成功返回 {@code true} 并记一条出库流水；
     * <b>库存不足返回 {@code code=200} + {@code data=false}，不记流水、不抛异常</b>——库存不足不是故障，
    ...
    RespData<Boolean> deductStock(@RequestBody StoreStockDeductDTO dto);
