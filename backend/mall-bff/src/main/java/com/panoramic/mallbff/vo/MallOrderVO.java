...
public class MallOrderVO {
    ...
    public static class Item {
        ...
        /**
         * 该商品（<b>按 SPU</b>）在本单里是否已评价——<b>只在订单详情出口下发</b>
         * （列表页不补：那会变成「一单一次跨服务调用」）。
         *
         * <p>同单里同一 SPU 的多个 SKU 行<b>同值</b>（评价挂在 SPU 上），前端按 {@code spuId} 归组后
         * 取任一行的即可。取法：拿该单号<b>一次</b>调 store 域取「本单已评价的 spuId 集合」，不逐行调。</p>
         *
         * <p>⚠ <b>可为 {@code null}</b>：store 域不可用时<b>静默降级</b>（评价态只是增强，
         * 不能因此让订单详情打不开），此时所有行都是 {@code null} = 「不知道」而不是「未评价」——
         * 前端不要渲染成「待评价」，入口照常可点，点了由服务端兜（域侧唯一键会拒重复评价）。</p>
         */
