...
| 模块 | 类型 | 端口 | 说明 |
|---|---|---|---|
| [common](common/) | 工具包（非服务） | — | 通用返回结构、异常处理、实体基类、自动填充等，供业务服务引用 |
| [gateway](gateway/) | 网关服务 | 8080 | Spring Cloud Gateway 响应式网关，统一入口与路由转发 |
...
- MySQL 8（库 `panoramic_mall`，表结构见 `goods-center/src/main/resources/db/schema.sql`）
...
```
前端(5173) ──/goods、/discovery──▶ gateway(8080) ──lb://goods-center──▶ goods-center(8081) ──▶ MySQL
                                        │
                                        └── Nacos(8848) 服务注册与发现
```
