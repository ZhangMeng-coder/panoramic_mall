...
- **以上这些跨服务约定（RespData 形状、身份头、Redis 键、熔断 4xx/5xx 分野、Nacos 加载矩阵、权限串一致性等）的完整登记与逐条核对方式见 [docs/contracts/cross-cutting.md](../docs/contracts/cross-cutting.md)** —— 共 16 条，每条标注定义位置、消费位置、破坏后果与静态核对方式。本节只讲原则，条目不在此重复。
