...
| [trade-center](trade-center/) | 业务服务（下沉纯域） | 8087 | 交易域：购物车 `trade_cart_item` + **订单**（DDD 三层；订单**已落库、下单接口已就绪**——三侧 10 条，⚠ 仅经内部 Feign 被端 BFF 调用、**不暴露公网路由**；三端 BFF 编排 / 前端 / 结算 / 评价 / Seata 属后续期，见 [todo.md](../todo.md)）。⚠ 全仓**唯一加载 Redis 的域**（加购去重与计数缓存，MySQL 仍是唯一事实源） |
