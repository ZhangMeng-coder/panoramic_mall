...
| [trade-center](trade-center/) | 业务服务（下沉纯域） | 8087 | 交易域：购物车 `trade_cart_item` + **订单领域模型**（DDD 三层，⚠ **无接口、不落库**，纯模型、仅单测验证；订单接口 / 落库 / 结账 / 评价属后续期）；不暴露公网路由，仅被 mall-bff 内部 Feign 调用。⚠ 全仓**唯一加载 Redis 的域**（加购去重与计数缓存，MySQL 仍是唯一事实源） |
