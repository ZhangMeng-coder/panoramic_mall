...
> 已接 **goods-center**（分类树）与 **store**（商品分页 / 筛选聚合）两个业务域；
...
| 内部依赖 | **已启用** `@EnableFeignClients(basePackages = {"com.panoramic.common.goods", "com.panoramic.common.store"})`；分类树经 `GoodsCenterClient` 调 goods-center，商品分页 / 筛选聚合经 `StoreClient` 调 store。编排集中在 `CatalogBffService`，统一走 `common` 的 `BffFeignCall`（下游故障降级为「…暂不可用」，业务 4xx 原样透传） |
