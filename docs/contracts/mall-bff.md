...
| 下单入参 | `MallOrderCreateDTO` = `source`（`DIRECT` / `CART`）+ `requestId`（客户端生成）+ `addressId`（**必填**）+ `items[{skuId, quantity}]` + `cartItemIds`（仅 `CART` 结算带，`DIRECT` 直购为空）。⚠ **传 `addressId` 而不是地址快照**：地址由本层经 customer-center 取回（顺带校验归属）后组装成快照传给 trade-center —— **trade-center 结构上调不到 customer-center**（每个域只依赖自己的 `<域>-interface`） |
