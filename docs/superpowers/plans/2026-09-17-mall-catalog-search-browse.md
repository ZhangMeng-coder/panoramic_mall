...
⚠ **`pageSize` 必须是 49，写死，不得用「一页塞满」式的大值**：TS 侧 `types/catalog.ts` 的 `pageSize` 是裸 `number` 不设上限，而后端 `BasePageVO` 强制 `@Min(1) @Max(100)`（Task 7 修复轮加的 `@Valid` 现在真的会拦），超 100 直接 400。49 = 7 列 × 7 行整行且 ≤100，两头都合。

⚠ **Pager 不会回告父级，钳越界的责任在本页面**（Task 8 复评 minor 2，已实测语义）：`Pager` 把 `current` 钳到 `[1, pageCount]` 只影响它自己的高亮，**不 emit**。所以本页面必须自己兜：结果集变小（改筛选、改关键词、从分类页跳来）导致当前页超出新 `pageCount` 时，**本页面要自己把 `page` 状态钳回范围并重新拉数据**。漏了就是：父级 `page=5`、新结果只有 2 页 → 分页器正确高亮「2」，父级仍发 `pageNum=5` → 列表空白而分页器显示在第 2 页，且点已高亮的「2」按设计是 no-op（`go()` 里 `page === current` 直接返回）→ **用户只能靠点「上一页」自救**。做法：拿到 `total` 后算 `pageCount`，若 `page > pageCount` 就把 `page` 归一到 `pageCount`（或直接回第 1 页）再发请求，并同步写回 URL query。

> 口径备忘（Task 8 已定的读法，**不要再当成歧义去问**）：`Pager` 的「最多 7 个」= **7 个槽位含省略号**（首尾恒在、当前页居中、省略号一侧或两侧）；`pageCount <= 1` 时 Pager 自隐。
...
```bash
cd /e/workspace/panoramic_mall
grep -rn "不调任何业务域\|不调业务域\|不调域\|一期只做账号\|一期未接入\|无 Feign 客户端\|没有 Feign 客户端\|一期没有\|属空转\|首页数据聚合[是属为]二期\|首页六个区块的数据仍全部静态" \
...
> ⚠ **关键词要用 `属空转`，不要用裸 `空转`，也不要写 `是空转`**（2026-09-18 实测修正）：修复轮自己会写下**正确的否定句**——「`feign-circuitbreaker.yml` **不再是空转**」（`backend/mall-bff/README.md:86`、`cross-cutting.md:175`），而它同时被裸 `空转` 和 `是空转` 命中（`不再` + `是空转`）。裸 `空转` 的门禁**永远不可能为空**，会逼着执行者把两句正确的话删掉——即「为了让检查通过而改坏正确内容」，正是下面那条 ⚠ 要防的。原假话的确切措辞是「属**空转**」，故以 `属空转` 精确区分。
>
> ⚠ **关键词表本身也漏过词**（两轮各漏一次）：初版漏了 `不调业务域`（`README.md:41` 用的正是这个说法，既不被 `不调任何业务域` 也不被 `不调域` 命中）。**做法：先想「这句话还有哪些说法」，跑一遍，再拿结果反推「有没有我没想到的说法」**，不要假定第一次的关键词表是全的。
