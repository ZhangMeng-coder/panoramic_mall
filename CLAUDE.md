...
**所有服务的对外契约统一登记在 [`docs/contracts/`](docs/contracts/README.md)**，不在各模块 README 或本文件里另立一份。三层：① 页面级（`admin.md` / `store-bff.md` / `mall-bff.md`）② 内部 Feign（`goods-center.md` / `store.md` / `trade-center.md`，后两者中 `trade-center.md` 待建）③ 跨服务隐式（`cross-cutting.md`，共 20 条），外加基础设施（`gateway.md`）。
...
- **详情页形态（`/goods/:id`）**：同一套骨架 + 面包屑（首页 › 分类 › 商品）→ 左 480px 图位（大图 + 缩略图）/ 右侧信息（名称 / 价格带 / 分类品牌店铺 / 规格选择）→ 下方商品详情正文。⚠ 两条硬口径：**详情正文按纯文本渲染**（`{{ }}` + `white-space: pre-wrap`，**不用 v-html**——店主可自由录入，v-html 等于给 C 端开注入口）；**不做「加入购物车 / 立即购买」**（后端没有购物车与下单接口，摆了就是点了没反应的按钮）。列表卡进详情走**整卡链接**（`.cat-card__hit` 铺满卡片的透明层），不是把 `<li>` 换成链接。
...
⚠ **401 一律提示登录并跳登录页**（`src/api/request.ts` 的 `sessionExpired`：清本地态 + 弹「请先登录」+ 带 `redirect` 跳 `/login`）——与 store / admin 两端行为一致，**不再是刻意的差异**。**唯一静默的 401** 是路由守卫刷新重建用户态的 `authApi.me()`（带 `silent401`）：公开首页上的重建失败不该把游客弹走，页面照常按未登录态渲染（顶栏自己会变成「请登录」）。改这块时别把 `silent401` 去掉，也别让静默面扩大到别的调用。
