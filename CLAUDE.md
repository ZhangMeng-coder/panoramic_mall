...
⚠ **内容范围**（商品卡 5 列 × 2 行、价格三层字号、角标 / 原价 / 销量位）与 `src/mock/*.ts` 里逐条探边界的假数据（各文件顶部的 `[探]` 注释），是刻意的基准，不要随手改小。⚠ 唯一例外是 **③ 全分类展示**：一级分类数量**已改由后端分类树决定**（`CategoryGrid.vue` 读 `/catalog/categories`，当前 9 项），不再来自本工程 mock；但「宫格正好铺满一行」这条基准仍然看它。
