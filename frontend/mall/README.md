...
页面域：首页（六区块）、搜索结果与分类商品列表、商品详情（含**评分**与**评价区**）、**购物车**、**我的订单**（列表 / 详情，支付 / **改收货地址** / **取消** / **仅退款** / **评价商品** 都在详情页内，**待支付倒计时**两页都有）、登录 / 注册、账号（个人资料 / 收货地址）。
...
- **评分与评价区**：星级组件是 [`src/components/StarRating.vue`](./src/components/StarRating.vue)（只读展示与可点选择**同一个组件**，
  样式 `.stars*` 落在 [`src/styles/catalog.css`](./src/styles/catalog.css)——跨页组件的样式跟**主要使用页**走的既有先例，同 `Pager` / `AddressPicker`）。
  ⚠ **评分可空**：`score` / `shopScore` 为 `null` 表示**无人评价** → **不渲染评分**（不显示 0、不显示占位）——
  「没人评过」与「评了 0 分」不是一回事。列表卡与详情页摆**商品评分**、详情页另摆**店铺评分**；
  星形按**四舍五入**取整，权威数字永远是它旁边那个小数。
  ⚠ 评价区（商品详情页底部）**只读**：C 端**没有回复入口**（回复是商户端的事）、不能改 / 删自己的评价，
  **也不提供星级筛选**。⚠ **分布必须取自分布接口**，不能从当前页算（那只会算出本页的）；域侧保证恒 5 行、含 0 人的星级。
  评价与回复文字一律 `{{ }}` 插值，**不许 `v-html`**——商品详情正文那条 HTML 链路的消毒前提是
  「店主自由录入 HTML」，评价没有这个前提。⚠ 评价区是**独立一份加载 / 错误态**（区块内提示 + 重试），
  取不到只让那一块空着，不拖垮商品详情。
- **评价入口在订单详情页**（`RECEIVED` 才有，见 [`src/views/order/OrderDetailView.vue`](./src/views/order/OrderDetailView.vue) 文件头 ⑪）：
  **按商品（SPU）评、不按明细行**——同单里同一 SPU 的多个 SKU 行只评一次，故入口按 `spuId` 归组后给一次。
  ⚠ `evaluated` 为 `null`（后端静默降级、没取到该标记）时**入口照常给**，重复提交由服务端拒——
  页面不猜，猜错就会藏掉一个本来能用的入口。提交面板**行内展开**（同支付面板；模态层只装选地址那一个）。
