...
| `/goods` | 商品管理 | **在售商品列表**：筛选（分类/品牌/上下架/关键字）+ 分页 + 新增/编辑/删除 + 「规格」弹窗维护 SKU 与上下架；列含**分类全路径**与**平台锁定状态**；仅店铺已通过（status=2）时侧栏出现并可访问 |
...
**分类列展示全路径**（2026-09-12）：列表分类列显示 `categoryPath || categoryName`（如「服饰 / 男装 / T恤」）。路径由 **store-bff 读时调 goods-center 解析**（域不持分类表），解析失败时该列为空、自动回退落库快照的分类名——**无需前端额外请求**。

**平台锁定（2026-09-12，只读呈现）**：商品可被平台管理员锁定，店铺端**只能查看、不能改**：
