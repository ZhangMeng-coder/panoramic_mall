...
import type { ShopInfo, ShopStatus } from '../types/shop'
 ...
 */
export const shop = ref<ShopInfo | null>(null)

export const shopStatus = computed<ShopStatus>(() => (shop.value ? shop.value.status : 0))

export const isApproved = computed(() => shopStatus.value === 2)

/** 拉取当前店主店铺并刷新本地状态（无店铺置 null） */
export async function fetchMyShop(): Promise<void> {
  try {
